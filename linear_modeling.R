...
#' @return A tidy dataframe with adjusted Beta, CIs, and Reference Level info.
run_multivariable_model <- function(df, dep_var, predictors) {
  ...
  # Fit the single multivariable linear model
  model <- tryCatch({
    lm(as.formula(formula_str), data = df)
  }, error = function(e) {
    stop(paste("The multivariable model failed to fit. Check for perfect collinearity.\nError:", e$message))
  })
  
  # Extract coefficient summary and CIs
  coef_summary <- summary(model)$coefficients
  ci <- suppressMessages(confint(model))
